...
function autoCommit(options, cb) {
    var force = false;
    var topicPayloads = this.topicPayloads;

    if (_.isBoolean(options)) {
        force = options;
    } else if (_.isObject(options)) {
        force = options.force;
        if (options.topicPayloads) {
            topicPayloads = options.topicPayloads;
        }
    }
    ...
    var commits = topicPayloads.filter(function(p) {
        return p.offset !== 0
    });
...
HighLevelConsumer.prototype.setOffset = function(topic, partition, offset) {
    ...
    this.emit("partition-offset-reset", {topic: topic, partition: partition, offset: offset});
};
